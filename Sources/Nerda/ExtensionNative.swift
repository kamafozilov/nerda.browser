...
// extensions may run it. Nerda reads the same files, runs the same program
...
enum ExtensionNative {
    ...
    static func connect(_ port: WKWebExtension.MessagePort, from extensionID: String) throws {
        ...
        port.messageHandler = { message, _ in
            ...
            if let asked = message as? [String: Any], let word = asked["__nerdaNative"] {
                ...
                port.sendMessage(["__nerdaNative": "here"], completionHandler: nil)
                ...
                if beating == nil {
                    beating = Timer.scheduledTimer(withTimeInterval: 25, repeats: true) { timer in
                        ...
                        port.sendMessage(["__nerdaNative": "alive"], completionHandler: nil)
                    }
                }
                return
            }
