...
enum ExtensionShims {
    ...
    nonisolated static let stamp = ".nerda-shim"
    /// Nerda Dev also lists what an extension's own code says went wrong.
    #if DEBUG
    ...
    /// carries — is Nerda's to say, never the package's: anything by those
    ...
    nonisolated static func prepare(_ folder: URL, fresh: Bool = false) throws {
        ...
        if fresh {
            for name in [stamp, ".nerda-added"] { try? files.removeItem(at: folder.appendingPathComponent(name)) }
        }
        ...
        // described by what it asked for, not by what Nerda gave it.
        var permissions = manifest["permissions"] as? [Any] ?? []
        let asked = Set(permissions.compactMap { $0 as? String })
        var added = (try? JSONSerialization.jsonObject(with: Data(contentsOf: folder.appendingPathComponent(".nerda-added")))) as? [String] ?? []
        ...
        if let data = try? JSONSerialization.data(withJSONObject: Array(Set(added)).sorted()) {
            try? data.write(to: folder.appendingPathComponent(".nerda-added"))
        }
            ...
            if let worker = background["service_worker"] as? String,
               let path = inside(worker, of: folder) {
                if var source = try? String(contentsOf: path, encoding: .utf8) {
                    ...
                    // Nerda puts its newer shim in its place.
    ...
    (() => {
      ...
      // popup's (Dark Reader).
      if (typeof location !== "undefined" && typeof history !== "undefined" && location.pathname.includes(".nerda-popup")) {
        try { history.replaceState(history.state, "", location.pathname.replace(".nerda-popup", "") + location.search + location.hash); } catch (e) {}
      }
      ...
      if (credentials && !Object.prototype.hasOwnProperty.call(root, "__nerdaCredentials")) {
        Object.defineProperty(root, "__nerdaCredentials", { value: credentials });
      }
      Object.defineProperty(root, "__nerdaShim", { value: true });
      ...
      const kept = new Set();
      try { Object.defineProperty(root, "__nerdaKept", { value: kept }); } catch (e) {}
      ...
      const native = (api, args) =>
        runtime.sendNativeMessage("nerda", { api, args: JSON.parse(JSON.stringify(args ?? [])) })
          .then((reply) => {
      ...
      if (worker && typeof root.WebSocket === "function" && runtime && typeof runtime.connectNative === "function") {
        ...
        class WebSocket extends EventTarget {
          ...
          #connect() {
            const port = connectNative("nerda.socket");
      ...
      if (worker && runtime && typeof runtime.connectNative === "function") {
        let found = null;
        try { found = runtime.connectNative("nerda"); found.disconnect(); } catch (e) {}
        ...
        if (portProto && eventProto && typeof portProto.postMessage === "function" && typeof eventProto.addListener === "function") {
          ...
          const ours = (m) => !!m && typeof m === "object" && "__nerdaNative" in m;
          ...
          const start = (port) => {
            ...
            const again = () => {
              ...
              try { post.call(port, { __nerdaNative: "here?" }); } catch (e) {}
              setTimeout(again, 100 * Math.min(tries, 5));
            };
            add.call(port.onMessage, (m) => {
              if (m && m.__nerdaNative === "here" && state.held) flush();
              ...
              if (m && m.__nerdaNative === "alive") { try { post.call(port, { __nerdaNative: "beat" }); } catch (e) {} }
            });
          ...
          put(portProto, "postMessage", function (message) {
            ...
            if (!state) {
              const native = !toPages.has(this) && this.sender == null && typeof this.name === "string" && !/^nerda(\.|$)/.test(this.name);
      ...
      // one — Safari's, as Nerda's tabs send — not the Chrome one the
      ...
      if (!inContent && typeof navigator !== "undefined" && !/ Chrome\//.test(navigator.userAgent)) {
        const chromeUA = navigator.userAgent.replace(/ Version\/[\d.]+.*$/, "").replace(/ Safari\/[\d.]+$/, "") + " Chrome/__NERDA_CHROME__ Safari/537.36";
        ...
        try {
          if (proto) {
            ...
            if (!("userAgentData" in navigator)) {
              const major = "__NERDA_CHROME__".split(".")[0];
              ...
              const high = {
                ...
                fullVersionList: brands.map((b) => ({ brand: b.brand, version: b.version === major ? "__NERDA_CHROME__" : b.version + ".0.0.0" })),
                uaFullVersion: "__NERDA_CHROME__",
              };
      ...
      if (worker && typeof root.importScripts === "function") {
        ...
        for (const p of __NERDA_SCRIPTS__) p.startsWith("-") ? empty.add(p.slice(1)) : shipped.add(p);
      ...
      // The tab an extension's framed page is in, asked once (see __nerdaToFrame).
      let ownTab = null;
      ...
      const channel = !inContent && !embedded && typeof BroadcastChannel === "function" ? new BroadcastChannel("nerda-messages") : null;
      ...
      const gather = (event, told) => {
        ...
        const dispatch = function (message, sender, respond, viaRelay) {
          ...
          if (message && message.__nerdaPing === true) {
          ...
          if (message && message.__nerdaToFrame) {
            const to = message.__nerdaToFrame;
            ...
            if (!ownTab) ownTab = Promise.resolve(runtime.sendMessage({ __nerdaCall: { space: "tabs", method: "getCurrent", args: [] } }))
          ...
          // gives it.
          if (message && message.__nerdaCall) {
            if (!background) return true;
            const { space, method, args } = message.__nerdaCall;
      ...
      if (embedded) {
        ...
        const ask = (space, method, args) => {
          ...
          return Promise.resolve(chrome.runtime.sendMessage({ __nerdaCall: { space, method, args: payload } })).then((reply) => {
      ...
      if (runtime && typeof runtime.sendMessage === "function") {
        ...
        const check = () => {
          ...
          const ask = () => Promise.race([ping.call(runtime, { __nerdaPing: true }), new Promise((r) => setTimeout(() => r("late"), 15000))]).catch(() => undefined);
          ...
          attempt(0).then((r) => {
            ...
            if (r === "pong" || heard >= started) heard = Math.max(heard, Date.now());
              ...
              if (__NERDA_VERBOSE__) native("debug.error", ["worker check: " + String(r) + " from " + location.pathname]).catch(() => {});
      ...
      const alsoFramed = (answer, tabId, message, options) => {
        ...
        const framed = Promise.resolve(nav.getAllFrames({ tabId })).then((frames) => {
          ...
          return Object.getPrototypeOf(runtime).sendMessage.call(runtime, { __nerdaToFrame: { tabId, urls, message } });
        }, () => undefined);
      ...
      const refuse = (what) => (...args) => {
        const callback = args.length && typeof args[args.length - 1] === "function" ? args.pop() : null;
        const error = new Error(what + " isn't available in Nerda");
      ...
      // shown in Nerda, so there is nothing for them to do.
      ...
      // The popup Nerda shows is a page of its own, known to WebKit as a
      ...
      // Nerda opens a popup itself (see Extensions.press), and has to know
      ...
      // rest still go in: one rule Nerda can't honour shouldn't cost an
      ...
      // Context menu entries for places Nerda has no menu for — the old
      ...
      if (!chrome.userScripts && wantsUserScripts && scripting && typeof scripting.registerContentScripts === "function") {
        ...
        root.__nerdaUserScriptMessage = (message, sender, respond) => {
          let keep = false;
      ...
      if (runtime && typeof runtime.connect === "function" && runtime.onConnect) {
        ...
        const number = (port) => {
          ...
          event.addListener.call(event, (message, ...rest) => {
            const tag = message && typeof message === "object" ? message.__nerdaPort : null;
          ...
          set(port, "postMessage", (message) => post.call(port, { __nerdaPort: [me, ++sent], message }));
    ...
    static func answer(_ message: Any, from context: WKWebExtensionContext, owner: Extensions) async throws -> Any? {
        guard let body = message as? [String: Any], let api = body["api"] as? String else {
            return ["error": "Not a Nerda message"]
        }
    ...
    private static func run(_ api: String, _ args: [Any], context: WKWebExtensionContext, owner: Extensions) async throws -> Any? {
        ...
        switch api {
        // MARK: bookmarks — Nerda's own, under Chrome's bookmarks bar ("1")
        case "bookmarks.getTree":
            return [root(browser.bookmarks.items)]
        case "bookmarks.getSubTree":
            guard let key = first as? String else { return [] }
            if key == "0" { return [root(browser.bookmarks.items)] }
            if key == "1" { return [bar(browser.bookmarks.items)] }
            return find(key, in: browser.bookmarks.items).map { [node($0.node, parent: $0.parent, index: $0.index, deep: true)] } ?? []
        case "bookmarks.getChildren":
            let key = first as? String ?? "1"
            if key == "0" { return [bar(browser.bookmarks.items, deep: false)] }
            let kids = key == "1" ? browser.bookmarks.items : (find(key, in: browser.bookmarks.items)?.node.children ?? [])
            return kids.enumerated().map { node($1, parent: key, index: $0, deep: false) }
        case "bookmarks.get":
            let keys = (first as? [String]) ?? (first as? String).map { [$0] } ?? []
            return keys.compactMap { key in
                find(key, in: browser.bookmarks.items).map { node($0.node, parent: $0.parent, index: $0.index, deep: false) }
            }
        case "bookmarks.getRecent":
            let count = (first as? Int) ?? 10
            return flat(browser.bookmarks.items).filter { !$0.node.isFolder }.suffix(count).reversed()
        ...
        case "bookmarks.search":
            ...
            return flat(browser.bookmarks.items).filter { hit in
                let n = hit.node
                if let wantURL, n.url?.absoluteString != wantURL { return false }
                if let wantTitle, n.title != wantTitle { return false }
                let hay = (n.title + " " + (n.url?.absoluteString ?? "")).lowercased()
                return words.allSatisfy { hay.contains($0) }
            }.map { node($0.node, parent: $0.parent, index: $0.index, deep: false) }
        case "bookmarks.create":
            let spec = first as? [String: Any] ?? [:]
            let parent = (spec["parentId"] as? String).flatMap(UUID.init(uuidString:))
            let url = (spec["url"] as? String).flatMap(URL.init(string:))
            let made = Bookmarks.Item(title: spec["title"] as? String ?? "", url: url, children: url == nil ? [] : nil)
            browser.bookmarks.add(made, to: parent, at: spec["index"] as? Int ?? .max)
            return find(made.id.uuidString, in: browser.bookmarks.items).map { node($0.node, parent: $0.parent, index: $0.index, deep: false) }
        case "bookmarks.update":
            ...
            browser.bookmarks.update(uuid, title: changes["title"] as? String, url: (changes["url"] as? String).flatMap(URL.init(string:)))
            return find(key, in: browser.bookmarks.items).map { node($0.node, parent: $0.parent, index: $0.index, deep: false) }
        case "bookmarks.move":
            guard let key = first as? String, let uuid = UUID(uuidString: key) else { throw Unsupported(what: "No such bookmark") }
            let target = args.count > 1 ? args[1] as? [String: Any] ?? [:] : [:]
            let folder = (target["parentId"] as? String).flatMap(UUID.init(uuidString:))
                ?? browser.bookmarks.all.first { $0.id == uuid }?.folder
            browser.bookmarks.move(uuid, to: folder, at: target["index"] as? Int ?? .max)
            return find(key, in: browser.bookmarks.items).map { node($0.node, parent: $0.parent, index: $0.index, deep: false) }
        ...
        case "history.getVisits":
            ...
            return url.flatMap { History.shared.visits[$0] }.map { page in
                [["id": page.url.absoluteString, "visitId": "1", "visitTime": page.last.timeIntervalSince1970 * 1000,
                  "referringVisitId": "0", "transition": "link"]]
            } ?? []
        case "history.addUrl":
            if let url = ((first as? [String: Any])?["url"] as? String).flatMap(URL.init(string:)) {
                History.shared.visit(url, title: "")
            }
            return nil
        case "history.deleteUrl":
            if let url = ((first as? [String: Any])?["url"] as? String).flatMap(URL.init(string:)) {
                History.shared.remove(url)
            }
            return nil
        ...
        // MARK: downloads — this run's, numbered from 1 in the order they came
        ...
        case "userScripts.list":
            return (try? JSONSerialization.jsonObject(with: Data(contentsOf: Extensions.folder(for: id).appendingPathComponent("_nerda/userscripts.json")))) ?? []
        case "userScripts.save":
            let folder = Extensions.folder(for: id).appendingPathComponent("_nerda", isDirectory: true)
        ...
        case "userScripts.world", "userScripts.worlds":
            let url = Extensions.folder(for: id).appendingPathComponent("_nerda/worlds.json")
        ...
        // MARK: permissions Nerda grants itself
        case "permissions.granted":
            return UserDefaults.standard.stringArray(forKey: "extensions.granted.\(id)") ?? []
        ...
        case "tabs.move", "tabs.discard", "tabs.activate":
            ...
            switch api {
            case "tabs.move":
                // Among the tabs of its kind, as the sidebar lists them.
                let wanted = args.dropFirst().first as? Int ?? -1
                let kind: (Tab) -> Bool = tab.isPinned ? { $0.isPinned } : Browser.listed
                let target = visible[wanted < 0 || wanted >= visible.count ? visible.count - 1 : wanted]
                let place = visible.filter(kind).firstIndex { $0 === target }
                browser.move(tab.id, pinned: tab.isPinned, to: place ?? .max, among: Browser.listed)
            case "tabs.discard":
                if tab.id != browser.selectedID { tab.sleep() }
            default:
        ...
        case "search.query":
            let spec = first as? [String: Any] ?? [:]
            guard let url = Address.search(spec["text"] as? String ?? "") else { return nil }
            switch spec["disposition"] as? String {
            case "NEW_TAB", "NEW_WINDOW": browser.open(url)
            default: browser.go(to: url)
            }
            return nil
        ...
        case "power.requestKeepAwake":
            ...
            if IOPMAssertionCreateWithName(kind, IOPMAssertionLevel(kIOPMAssertionLevelOn), "An extension in Nerda" as CFString, &assertion) == kIOReturnSuccess {
                awake[id] = assertion
            }
            return nil
        ...
        case "power.reportActivity":
            var assertion: IOPMAssertionID = 0
            IOPMAssertionDeclareUserActivity("An extension in Nerda" as CFString, kIOPMUserActiveLocal, &assertion)
            return nil
        ...
        case "sessions.restore":
            throw Unsupported(what: "Nothing to restore")

        // MARK: top sites — the most visited in history
        case "topSites.get":
            var sites: [String: (url: URL, title: String, count: Int)] = [:]
            for page in History.shared.visits.values {
                guard let host = page.url.host() else { continue }
                sites[host, default: (page.url, page.title, 0)].count += page.count
            }
            return sites.values.sorted { $0.count > $1.count }.prefix(10).map {
        ...
        case "readingList.addEntry", "readingList.removeEntry", "readingList.updateEntry":
            throw Unsupported(what: "Nerda has no reading list")
        ...
        case "tabGroups.get", "tabGroups.update", "tabGroups.move":
            throw Unsupported(what: "Nerda has no tab groups")
        ...
        default:
            throw Unsupported(what: "\(api) isn't available in Nerda")
        }
    }
        ...
        let prelude = #"""
          const chrome = (() => {
            ...
            return { runtime: {
              ...
              sendMessage: (message, ...rest) => runtime.sendMessage({ __nerdaUserScript: true, message }, ...rest.filter((r) => typeof r === "function" || (r && typeof r === "object"))),
              connect: (info) => runtime.connect({ ...(info || {}), name: "nerda-us:" + ((info && info.name) || "") }),
            } };
          })();
          const browser = chrome;
        """#
        let text = #"""
        /* Nerda: a user script (chrome.userScripts) */
        nerda_user_script: {
          const __nerdaHref = location.href;
          const __nerdaGlob = (g) => new RegExp("^" + g.replace(/[.+^${}()|[\]\\]/g, "\\$&").replace(/\*/g, ".*").replace(/\?/g, ".") + "$");
          const __nerdaIn = \#(json(script["includeGlobs"] ?? [])), __nerdaOut = \#(json(script["excludeGlobs"] ?? []));
          if ((__nerdaIn.length && !__nerdaIn.some((g) => __nerdaGlob(g).test(__nerdaHref))) || __nerdaOut.some((g) => __nerdaGlob(g).test(__nerdaHref))) break nerda_user_script;
        \#(userWorld ? prelude : "")
        \#(code)
        }
        """#
        let name = "us-" + SHA256.hash(data: Data(text.utf8)).prefix(8).map { String(format: "%02x", $0) }.joined() + ".js"
        let dir = folder.appendingPathComponent("_nerda", isDirectory: true)
    ...
    /// launches as Chrome keeps it. Nerda acts on one of them — an
    ...
    /// chrome.browsingData, from what WebKit and Nerda keep.
    ...
    static func openPanel(_ context: WKWebExtensionContext, owner: Extensions) {
        ...
        owner.browser?.open(url)
    }

    // MARK: - bookmarks, as Chrome shapes them

    private typealias Hit = (node: Bookmarks.Item, parent: String, index: Int)

    private static func flat(_ nodes: [Bookmarks.Item], parent: String = "1") -> [Hit] {
        nodes.enumerated().flatMap { index, n -> [Hit] in
            [(n, parent, index)] + flat(n.children ?? [], parent: n.id.uuidString)
        }
    }

    private static func find(_ key: String, in nodes: [Bookmarks.Item]) -> Hit? {
        flat(nodes).first { $0.node.id.uuidString == key }
    }

    private static func node(_ n: Bookmarks.Item, parent: String, index: Int, deep: Bool) -> [String: Any] {
        var out: [String: Any] = ["id": n.id.uuidString, "parentId": parent, "index": index, "title": n.title,
                                  "dateAdded": 0, "syncing": false]
        if let url = n.url { out["url"] = url.absoluteString }
        if n.isFolder {
    ...
    private static func bar(_ roots: [Bookmarks.Item], deep: Bool = true) -> [String: Any] {
    ...
    private static func root(_ roots: [Bookmarks.Item]) -> [String: Any] {
        ["id": "0", "title": "", "dateAdded": 0, "syncing": false, "children": [bar(roots)]]
    }

    private static func visit(_ page: History.Visit) -> [String: Any] {
        ["id": page.url.absoluteString, "url": page.url.absoluteString, "title": page.title,
         "lastVisitTime": page.last.timeIntervalSince1970 * 1000, "visitCount": page.count, "typedCount": 0]
    }
}
...
enum ExtensionAuth {
    ...
    /// Closing the sign-in's tab is saying no (see Extensions.follow).
    static func tabsChanged(_ tabs: [Tab]) {
