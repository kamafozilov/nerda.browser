...
struct AddressBar: View {
    ...
    var body: some View {
            ...
            Group {
                if browser.editingAddress, let tab {
                    CommandBar(
                        ...
                        inline: true,
                        go: { url in
                            browser.selected?.webView.load(URLRequest(url: url))
                    ...
                    AddressField(site: tab?.site, edit: browser.editAddress)
                }
            }
            .padding(.leading, 4)
        }
        ...
        .background(
            color.map { Color(nsColor: $0) } ?? Palette.ground,
            in: UnevenRoundedRectangle(topLeadingRadius: BrowserView.cornerRadius,
                                       topTrailingRadius: BrowserView.cornerRadius, style: .continuous)
        )
        ...
        .overlay(alignment: .bottom) {
            ZStack(alignment: .bottom) {
                ...
                LoadLine(loading: loading, progress: tab?.progress ?? 0, site: tab?.site)
            }
        }
    }
...
private struct AddressField: View {
    ...
    var body: some View {
        ...
        .buttonStyle(.plain)
        .accessibilityLabel(site?.absoluteString ?? "Search or enter address")
        .onHover { hovering = $0 }
