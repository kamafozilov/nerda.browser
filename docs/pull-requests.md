...
A change that shows on screen shows in the pull request; the pictures are what the owner looks at first. **Everything the pull request changes on screen has its picture**: each screen of a new feature, each place a fix touches. One picture of the last thing done is not enough: whatever isn't shown isn't reviewed.
...
They are taken in the test VM, never on the owner's Mac (see [AGENTS.md](../AGENTS.md)), into `build/screenshots/`, one file per picture, named for what it shows and numbered in the order to read them. A before and an after of one thing share the name:
...
1. For something that changes, before touching the code: `./vm.sh open`, bring Nerda to the state that shows it, `./vm.sh shot build/screenshots/3-extensions-menu.before.png`. Forgot? `git worktree add /tmp/nerda-base main`, the same steps from there, then `git worktree remove /tmp/nerda-base`.
2. After the change: `./vm.sh open`, the same steps, `./vm.sh shot` each picture into `build/screenshots/`.
...
The change isn't finished until every picture is in `build/screenshots/`; from there, they reach the pull request on their own:
...
- **The Screenshots check** (`.github/workflows/screenshots.yml`) runs on every pull request as it opens, is edited or gets new commits. It puts every uploaded picture into the Screenshots section, each under its caption, a before and an after side by side, whatever was written there, and fails a pull request that has none and doesn't say "Nothing on screen changes." A picture dragged into the body on GitHub counts too.
